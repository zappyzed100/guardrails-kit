...
この3＋1レンズを判定可能な14条に固定したのが **`GOALS.md`（目標の正本）**、それを実現する
機構の契約が **`GUARDRAILS.md`（契約の正本）**、言語固有の具象値が
**`bindings/catalog.md`（バインディングの正本）**——という三層構成。迷ったらこの順で読む。

## このキットの中身

このキット（v2）は GUARDRAILS.md の各機構（§1〜§9・§12）の**言語なし実装**です。
言語固有値は `bindings/catalog.md` の検証済み列（TS/React・Python・Dart/Flutter・Rust）
に集約してあり、Claude Code が GUARDRAILS.md §11 の手順で採用列を充填します。ユーザーがやることは
...
チェーンの中で断続的にタイムアウトするという報告を受け、原因を実測で特定・是正した回
（正本: GUARDRAILS.md §7.7・§10 Phase 32）。
...
## v2.21 での変更点（v2.20 からの整備。根拠は `GOALS.md` のG）
...
- **新規 `CUSTOMIZE.md`**（root）: 導入後にカスタムできる7項目の**索引**。中身を
  複製せず、GUARDRAILS.md の該当節へのポインタに徹する（複製は正本の分裂——Serena の
  `.serena/memories` 不採用と同じ理由でG5違反）。
- **導線を3箇所に追加**: `install_kit.py` の完了メッセージ／`BOOTSTRAP.md` の完了注記／
  両PROMPTファイルのStep 10報告フォーマット——ブートストラップが完了する瞬間に必ず
  1回は目に入るようにした。
- ファイル名はASCII（`CUSTOMIZE.md`）——日本語ファイル名はzip展開時の文字化けで
  メタ除外をすり抜ける事故が過去に実測されている（`README_SETUP.md` 改名の理由と同じ
  — G1）。

## v2.20 での変更点（v2.19 からの増築。根拠は `GOALS.md` のG）

セッション内の設計対話から出発した回（正本: GUARDRAILS.md §8.4・§10 Phase 31）。
...
## v2.18 での変更点（v2.17 からの増築。根拠は `GOALS.md` のG）
...
## v2.17 での変更点（v2.16 からの増築。根拠は `GOALS.md` のG）
...
## v2.16 での変更点（v2.15 からの整備。文書のみ・根拠は `GOALS.md` のG）
...
## v2.15 での変更点（v2.14 からの整備。文書のみ・根拠は `GOALS.md` のG）
...
## v2.14 での変更点（v2.13 からの整備。根拠は `GOALS.md` のG）
...
- **整合性監査（機械監査スクリプトで横断検査→所見8件を全て修正）**（G4/G5）:
  ...
  反映漏れ） ④ missing-required の列挙に台帳 BOOTSTRAP.md を明記 ⑤ §11 Step 2 見出しの
  旧「スクリプト4本」（v2 は全スクリプト同梱済み）を是正 ⑥ PROMPT の列ID例示 @4→@6・
  ファイル表に BOOTSTRAP.md 行を追加 ⑦ 不採用記録の「正本は2文書」を v2.10 以降の構成
...
## v2.13 での変更点（v2.12 からの増築。根拠は `GOALS.md` のG）
...
| プロセスファイル一式（Spec Kit の specify/plan/tasks・Kiro の requirements/design/tasks） | 不採用——チェックリストは AI 解釈で保証なし（Fowler）。本キットは同じ目的を門で達成済み（plan は検査5、DoD は BOOTSTRAP 監査、constitution は GOALS.md＋検査3） |
| constitution 概念そのもの | 採用済みと判定——GOALS.md（G引用の機械検査つき）が同等以上。名称だけの輸入はしない |
| GSD・Ralph Loop（メタプロンプト/ループ実行） | 対象外——実行時のオーケストレーションはキットの層（リポジトリ内の門）の外。プロンプト集の常駐は v2.11 と同根で不採用 |
| Beads・Memory Bank 系の台帳/メモリ | 不採用再確認——§13 中央メモ禁止。タスク台帳の正当な形は plan.md と BOOTSTRAP.md（検証可能な範囲のみ） |
...
## v2.12 での変更点（v2.11 からの増築。根拠は `GOALS.md` のG）

「LLM がプロンプト1つでキットを展開できるか懐疑的」（ユーザー提起）への回答の回。
...
- **ブートストラップ監査 `check-bootstrap`（hard・§3.5 新設）**（G7/G4/G2）: 進捗の正本を
  ルート **`BOOTSTRAP.md`（台帳）** に一本化し（状態 🚧/✅/—＋固有名詞リストC）、
...
## v2.10 での変更点（v2.9 からの増築。根拠は `GOALS.md` のG）
...
- 文書の更新（G5/G13）: §0 全体像・一次対応表（Stop 行を新設・red-first 行を更新）、
  ...
  カタログ・GOALS.md の変更なし（両機構とも既存Gの強化——列値も判定列も不変）。

## v2.8 での変更点（v2.7 からの増築。根拠は `GOALS.md` のG）

- **第4レンズ「意図の保存」＝G14「意図の複利」の新設と `feat-without-plan` の hard 昇格**
  （G14/G7/G5）: v2.6 から soft（表示のみ）で運用してきた検査5を **exit 1 の hard** へ
  昇格し、根拠を GOALS.md の**レンズ4**として憲法化した——**決定点①を案Aで確定**
...
- **同時改修の3点セット**（旧計画が「§1-7 で実測済みの罠」と警告していた漏れ——同一の
  ...
  `G(1[0-4]|[1-9])` へ（G14 引用が検査3で偽陽性にならない） ② GOALS.md ヘッダ
  「G1〜G13」「13条」→「G1〜G14」「14条」＋レンズ4新設・G5 行の分担整理（同梱強制は
  G14 へ移管、G5 は「正本が単一」に純化） ③ README・GUARDRAILS.md（§0 一次対応表・
...
## v2.7 での変更点（v2.6 からの増築。根拠は `GOALS.md` のG）
...
- 文書の更新（G5/G13）: §0 全体像・一次対応表に1行、§3.4 検査2・§9.3 に相互参照、
  **§5 に red-first ジョブの契約**、§10 Phase 18 を ✅ 化（実装時確定3点——worktree の
  ...
  GOALS.md G10 行に判定・機構を追記（✅→✅/（列））。`.gitignore` キット区画に
  `.red-first-*/` を追加（worktree 残骸で porcelain が汚れない——§2b の session/ と同じ理由）。

## v2.6 での変更点（v2.5 からの増築。根拠は `GOALS.md` のG）
...
- **世代交代 API 検査 `deprecated-api`**（G4/G13/G7）: LLM が訓練カットオフの都合で
  ...
  **テスト内限定でなく全コード走査**。列値の出典規律（①ベンダー公式 AI プロンプト
  ...
  ではない——`LOG_EXIT_PREFIXES` と同じ境界。GUARDRAILS.md §3.3）。
...
- **feat⇔plan 対 `feat-without-plan`（soft・表示のみ）**（G5）: レイヤー直下
  ...
  **決定点①は未決**——hard 昇格（G14「意図の保存」新設）は soft 運用でノイズ率を
  実測してからのユーザー判断（機械の実績が先、憲法改正は後——GUARDRAILS.md §10）。
...
- 文書の更新（G5/G13）: §0 全体像・一次対応表に3機構の行、**§2c 新設**、§3.3 に
  ...
  GOALS.md の G5/G7 行に新機構を追記。

## v2.5 での変更点（v2.4 からの増築。根拠は `GOALS.md` のG）
...
- **編集直後リント（PostToolUse 第2段）**（G4/G11）: 整形（第1段・自動修正系）の直後に
  ...
  1コマンド**として配線済み（順序が実行環境の仕様に依存しない — GUARDRAILS.md §1）。
...
## v2.4 での変更点（v2.3 からの増築。根拠は `GOALS.md` のG）
...
- **ターン終了ゲート（Stop フック）**（G7/G2）: 実行規律7「途中でターンを終えない」を
  ...
  終了不能にしないため。契約は GUARDRAILS.md **§2b（新設）**。
...
## v2.3 での変更点（v2.2 からの増築。根拠は `GOALS.md` のG）
...
- **メタ3ファイルをリポジトリに入れない**（G3）: 本書と PROMPT 2本は「インストールの説明書」
  であり、配置後のリポジトリには残らない（`GUARDRAILS.md` §11 が手順の正本として残る）。

## v2.2 での変更点（v2.1 からの増築。根拠は `GOALS.md` のG）
...
## v2 での変更点（v1 からの増築。根拠は `GOALS.md` のG）
...
- **目標の正本 `GOALS.md`**（G1〜G14）: キット・規約への変更はGの引用が必須。
...
```
リポジトリルート/
├── GUARDRAILS.md                     … 契約の正本（言語なし）
├── GOALS.md                          … 目標の正本（G1〜G14。変更はGを引用）
├── bindings/
│   └── catalog.md                    … 検証済みバインディング列（言語固有値の正本）
├── BOOTSTRAP.md                      … ブートストラップ進捗台帳（✅ は監査器が再実行検証 — v2.12）
...
- **移植元プロジェクトのルート `CLAUDE.md`（または `AGENTS.md`）原本を持っている場合は、
  ...
  v2.10 以降 `AGENTS.md` 側へ移設して完成させる——テンプレートは GUARDRAILS.md §6 の
...
1つでも素通りしたら、その項目は 🚧 に戻して再実装させる（GUARDRAILS.md §10 実行規律 4 の監査ルール）。
...
- `.claude/settings.json` の `permissions.deny` は**前方一致の第二防壁**。引数の順番を変えた
  ...
  回帰保護され、`dev.py probe "<cmd>"` で実行前に照会できる（v2.4 — GUARDRAILS.md §2）。
