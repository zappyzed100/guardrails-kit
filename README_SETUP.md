...
| 充填先 Python 4ファイル（repo_scan / dev / post_edit_format / post_edit_lint）の旧版 | `UPGRADED`——ただし管理区画（`>>> GUARDRAILS BINDING >>>`）の中身＝採用列の充填は**既存を引き継ぐ**（v2.42・Phase 44）。区画マーカーが無い旧版は `CONFLICT:unmarked-binding`（充填の黙失防止） |
