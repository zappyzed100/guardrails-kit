...
- CODEOWNERS更新が既存の独自規則を消し、最初のowner 1名へ全パスを平坦化する欠陥を修正。
  専用管理区画で独自規則・複数owner・パス別ownerを保持する。
...
- CODEOWNER承認後の追加pushで古い承認を再利用する経路を塞ぐため、stale review失効または
  last push approvalも外部監査の必須条件へ追加した。

## v2.50 での変更点（workflow自己改変と監査漏れの封鎖。根拠は `.guardrails/GOALS.md` の G7/G9/G10）

- `pull_request_target` で既定ブランチのコードだけを実行する `workflow-integrity` を追加。
  PR head は checkout / 実行せず Git blob として読み、信頼済みworkflow・検査器・CI workflow
  全体の変更を拒否する。言語別jobを `run: true` にする自己改変も通らない。
- required checks は4コアjobと採用した全言語別jobを要求し、ruleset bypass と旧来保護の
  admin非強制も未保護として検出する。CIの `push` は既定ブランチ名に依存しない。
- workflow更新時だけは人間レビュー後に `workflow-integrity` のrequired指定を一時解除し、
  同じPRをmerge後ただちに戻す。直接pushは使わない。

## v2.49 での変更点（PR 必須契約の曖昧さ解消。根拠は `.guardrails/GOALS.md` の G7/G9/G10）

- 新規向けプロンプトだけが「Step 9 以降は 1 PR」としていた矛盾を解消し、全経路を
  **1 Step = 1コミット = 1ブランチ = 1PR**へ統一。既定ブランチへの直接 push を禁止した。
- PR は好みのワークフローではなく、Web 編集・フック未導入マシンの変更に required checks を
  **合流前**に掛ける境界。`push` は合流後の事後検知なので代替にならない、と正本・
  README・テンプレへ明記した。
- Step 9 の外部設定監査を、当時の3コア required checks の登録だけでなく「PR 必須」ルールまで
  実測するよう拡張。リポジトリ内の心得だけで必須を名乗る監査空白を塞いだ。
...
- 新規発見は1系統のみ: **合流の門（マージキュー）が守るマージスキュー**（個別に緑の
  ...
  required checks がそのまま前提になる）。既設の `push` 再実行が「即検知・非予防」の
  部分防御であることも明文化。
...
展開もコピーも不要——実際の配置は Claude Code / Codex が同梱の `scripts/install_kit.py` で
...
`.claude/settings.json`・`.codex/hooks.json`・`.pre-commit-config.yaml`・CI などは既存リポジトリに高確率で
...
| `.gitattributes`・`.pre-commit-config.yaml`・`.claude/settings.json`・`.codex/hooks.json` が既存 | 検証条項を満たせば `KEPT`（既存維持）、満たさなければ**上書きせず** `CONFLICT`＋解消ヒント |
| キット系統ファイルの旧版（コミット済み・クリーン） | `UPGRADED`（旧内容は git 履歴が安全網。未コミット変更があれば `CONFLICT` で停止） |
| 充填先 Python 4ファイル、YAML 2ファイル（pre-commit / CI）、CODEOWNERSの旧版 | `UPGRADED`——名前付き管理区画とCODEOWNERの独自規則・複数/パス別ownerを**既存から引き継ぐ**（v2.53）。Python/YAMLの区画マーカー、またはCODEOWNERSの実ownerが無い旧版は `CONFLICT:unmarked-binding`（充填の黙失防止） |
...
追加モード（v2.42——いずれも書き込みなし）: `--detect` = 採用列の候補をマニフェストから
提示（Step 0 の前段・キット展開不要）／ `--diff` = 適用した場合の判定と差分行数の
プレビュー／ `--check` = ドリフト検出（全て OK/KEPT/SKIPPED なら exit 0——CI から
「導入先が新版に追随しているか」を機械判定できる）。

インストール後の姿（メタ——本書・`README.md`・PROMPT 4本（新規/既存/更新/Codex）・
`surveys/`・`docs/plans/`——は移植先へは入らない）:
...
├── .codex/
│   ├── hooks.json                    … Codex の PreToolUse/PostToolUse/SessionStart/Stop 配線（`/hooks` で信頼）
│   └── hooks/
│       ├── codex_hook_adapter.py     … Codex の apply_patch から対象パスを抽出する橋渡し
│       └── （防壁本体は `.claude/hooks/` を共用）
├── .github/
│   └── workflows/
...
└── scripts/
    ...
    ├── check_guard_corpus.py         … §2: コーパス再生＋probe 実体
    ├── check_codex_hooks.py          … Codex フック設定・アダプタの回帰検査
...
- フックはPythonで起動するため、手動配置でも `chmod +x` は不要。
...
| ツール | 用途 | 備考 |
|---|---|---|
| git + GitHub リモート | Step 9 の CI 実測に必須 | リモート未作成ならエージェントに `gh` で作らせてもよい |
...
1. Claude Codeなら新規 `PROMPT_claude_code.md`／既存 `PROMPT_claude_code_existing.md`、Codexなら
   `PROMPT_codex.md` を開き、必要な入力を埋めて全文をコピーしておく
   （**先にコピーする**——インストール成功時に zip は自動削除されるため）。
2. リポジトリルートで選んだエージェントを起動し、コピーしたプロンプトを最初のメッセージとして
   貼り付ける。エージェントが `install_kit.py` で配置（衝突があれば解消）まで行う。
3. Claude Codeなら `.claude/settings.json`、Codexなら `.codex/hooks.json` の配置直後に `/hooks` で
   返信を」と依頼される → `/hooks` で PreToolUse: Bash・Edit|Write|MultiEdit /
   PostToolUse: Edit|Write|MultiEdit / Stop / SessionStart の4種5エントリが有効かを
   確認し、信頼確認が出たら内容を見て承認して続行する。Codexは `apply_patch` を編集として
   表示する（アダプタが対象パスを抽出する）。
...
あとはエージェントが .guardrails/GUARDRAILS.md §11 の Step 0→10（骨格作成 → スクリプト調整 →
...
- `uv run scripts/check_guard_corpus.py` → `[guard-corpus] 全76行 PASS`。guard の規則を
...
- ファイルを1つ**人間の手で**書き換えたまま新しいセッションを開始し、そのファイルを
  Claude CodeまたはCodexに編集させてみる → ブロックされる。`git stash`（または commit）→ 通る
  （§2c 所有権ガード——自動解除の確認）。
...
- PR を1本作る → CI の checks / テストジョブがすべて緑。既定ブランチへの直接 push は
  サーバー側の「PR 必須」ルールで拒否される。
...
- **`AGENTS.md`・`CLAUDE.md`・`STRUCTURE.md` を .gitignore に入れない**（判断ごと記録）:
  ...
  よいのはセッション・一時状態（`.claude/session/`・`.codex/session/`・`.red-first-*/`——同梱済み）だけ。
