...
| 充填先 Python 4ファイル、YAML 2ファイル（pre-commit / CI）、CODEOWNERSの旧版 | `UPGRADED`——名前付き管理区画とCODEOWNER実値を**既存から引き継ぐ**（v2.51）。区画マーカー/実ownerが無い旧版は `CONFLICT:unmarked-binding`（充填の黙失防止） |
