...
## 列: ts-react-web@10 — TypeScript + React（Web/PWA・Vite・Supabase想定）【要実測】
...
## 列: python-uv@9 — Python（uv・CLI/バックエンド）【要実測】
...
pre-push/CI は ruff check・pytest を ts 列と同じ形で並べる。
...
## 列: dart-flutter@7 — Dart（Flutter・app/ 層）【ゲート系=移植元で実測済み／ランタイム系=要実測】
...
## 列: rust@8 — Rust（engine/ 層・ソルバー等）【ゲート系=移植元で実測済み／ランタイム系=要実測】
...
- **機械充填マーカー（v2.45・Phase 47）**: Python 4対象（repo_scan / dev /
  post_edit_format / post_edit_lint）への paste-block は、直前行に
  `<!-- FILL <対象パス> -->` を付ける——`scripts/fill_bindings.py <列ID@版>` がこれを
  抽出して管理区画へ自動適用する。新しい列・新しいブロックを足す時はマーカーを忘れない
  （マーカー無しのブロックは機械充填の対象外＝手貼りに戻る）。YAML 系は対象外（手動統合）。
