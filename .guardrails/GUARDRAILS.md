...
- **BINDING 充填のマーカー包み＋機械移植**（G13——v2.38 登録・**v2.42 で主要部を前倒し
  発動**）: Phase 44 が Python 充填先4ファイル（repo_scan / dev / post_edit_format /
  post_edit_lint）へ `# >>> GUARDRAILS BINDING >>>` 区画を導入し、インストーラの
  UPGRADED が区画の中身だけを既存から引き継ぐ形で実装した（前倒しの判定理由 = ユーザー
  指示＋`--diff`/`--check` が区画を前提にすると精度が上がる——判断の記録は
  surveys/SURVEY_HARNESS_TOOLS.md §1）。**残余 = YAML 系**（.pre-commit-config.yaml /
  guardrails-ci.yml）: ユーザー統合が kit 区画の内側（repos: / jobs: 配下）に入る構造の
  ため区画スプライスが安全でない——従来どおり検証条項 KEPT＋installer-token-drift。
  残余のトリガー = YAML 側の統合事故（KEPT 検証をすり抜けた設定欠落）の実測。
...
1つ選び、対象ファイルへ `BINDING-SOURCE: 列ID@版` を刻印する——§12.7。paste-block は
各ファイルの管理区画 `>>> GUARDRAILS BINDING >>>` の**内側**へ貼る——区画は更新時に
そのまま引き継がれる — Phase 44）。検証済み列が
...
- **`scripts/dev.py` が全プロジェクト共通の動詞**を提供する:
  ...
  `selftest` / `doctor`（後2つは v2.42・Phase 44）。
