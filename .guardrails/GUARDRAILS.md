...
2. **PR 内の workflow 自己改変**: 通常の required checks は PR ブランチ側の定義なので、
   ...
   `.github/CODEOWNERS` とcode owner review必須設定を併用する。一人開発では唯一の人間を
   CODEOWNERにし、PR作成主体を権限を絞ったmachine user/GitHub Appへ分離する。
...
- ✅ `binding-dead-path` — 充填したパス/prefix 型バインディング（`LOG_EXIT_FILES`・
  ...
  「見える猶予」
- ✅ `chronic-soft-violation` — 同一箇所の `file-too-long` / `dir-too-crowded` が
  **今回の実行でも現に違反中**かつローカルの違反ログ（`.guardrails/violations.jsonl`——
  gitignore済み・§3.6）上で `CHRONIC_SOFT_THRESHOLD`（既定5）回以上の `check-structure`
  実行にまたがって出続けている（v2.61・Phase 63）。関心事分割（AGENTS.md §1/§2）は
  「理由があれば1回のsoft警告は見送ってよい」を前提に行数/個数の機械分割を緩めた代わりに、
  無理由の放置がそのまま積み重なる経路を塞ぐセカンダリ検査（G3/G9）。**「現に違反中」の
  絞り込みが必須**——台帳は追記専用で解消済みの過去記録も残るため、絞り込みが無いと
  既に関心事で正しく割った/削除済みの箇所を古い記録だけで鳴らし続ける（実装時の
  dogfooding で実測——過去のDoDプローブ用フォルダの記録がログに残り誤検知した）。
  ローカル台帳が土台のため、同じ開発機での連続実行にのみ効く——新規clone・CI では台帳が
...
- **`scripts/check_bootstrap.py`**（pre-commit の `check-bootstrap`——`files: ^\.guardrails/BOOTSTRAP\.md$`
  ...
  - `bootstrap-false-done`: **✅ の Step ごとに、検証可能なアサーションをその場で再実行**
    ...
    **不在も検証不能も失敗**（v2.54・Phase 56）——Step 9は外部防壁の正の証拠を取れて初めて
    ✅にできる。rulesets/旧来保護の片方で全要件を証明できれば、他方の照会不能だけでは
    失敗にしない。CI 再監査で確定判定を得るには **rulesets 側での登録を推奨**する。
...
| §1-3 | ファイル500行・フォルダ7ファイル・ヘッダー一行の規約——軸は行数/個数でなく関心事。500行/7ファイルは分割要否の代理指標(soft)であり、LLMの読みやすさに効く本命は依存方向強制(§5)・ノイズ源の分離・役割ヘッダー(§3)・機械索引/契約(§4)・テスト名という契約 | §3.3のsoft検査 |
...
- **§4 文書の書式規約**（v2.62・**機械検査なし**——執筆スタイルであり判定列を持たないため
  `.guardrails/GOALS.md` には追加しない。強いて挙げれば G3「文脈の最小十分」の傍証:
  拡張子で「エージェントの必読契約」と「人間だけが読めばよい文書」を分け、後者を
  ブートストラップ手順の読み込み対象から自然に外す）: 人間だけが読む文書（導入手順・
  調査ノート・変更履歴）は HTML、§記法・grep索引・自動生成で構造的に参照される契約文書
  （AGENTS.md・CLAUDE.md・本書・`.guardrails/GOALS.md`・`bindings/catalog.md`・`STRUCTURE.md`）は
  Markdown のまま。**例外**: ルート `README.md` は GitHub 等のリポジトリトップページ自動
  プレビューのため Markdown のまま据え置く。
...
- 採用した各レイヤーの `CLAUDE.md` — その層だけに通用するテスト手順・デバッグ手順・
  ...
  **最優先で書くべき内容は「知らなければ必ず踏む罠」**（例: 「ある値はこのモードでは
  効かない」「この上限は特定の状態を意味しない」「ある近似は特定の条件でしか下界にならない」
  の類）——散文の説明より、踏んだ本人しか知らない地雷の列挙が事故を最も減らす。
  行数・ファイル数の規約(§1-3)より効く投資はこちらであって、フォルダを分けること自体ではない。
...
| 54 | 配布元CODEOWNERS分離＋workflow供給網固定 | §3.3 §5 §11 Step 9・catalog | ✅（v2.52 同梱） |
| 55 | CODEOWNERS更新時の独自規則保持＋GitHub owner解決実測 | §3.5 §11 Step 9・更新 | ✅（v2.53 同梱） |
| 56 | Step 9外部設定監査の検証不能fail-closed化 | §3.5 §11 Step 9 | ✅（v2.54 同梱） |
| 57 | Docker/local Actionの可変・保護外参照封鎖 | §5 §11 Step 9 | ✅（v2.55 同梱） |
| 58 | 全PR承認1件＋ゲート変更CODEOWNER承認の二層化 | §3.5 §11 Step 9 | ✅（v2.56 同梱） |
...
- **受入検証の残（実機未検証——§2d の検証根拠タグと同じ扱い）**: 「CI 上の GITHUB_TOKEN
  ...
  実測済みへ更新する。
- **後続是正（v2.54・Phase 56）**: 上記v2.35〜v2.36の「検証不能は表示して素通し」は、
  Step 9を✅にしないという外部防壁の契約と矛盾するため撤回。不在だけでなく、正の証拠を
  得られない状態も失敗へ変更した。片系統で全要件を証明できれば他方の403は許容する。
...
- 実装: `verify_required_checks` を3コアジョブの登録確認から「PR 必須＋3コアジョブ」の確認へ
  ...
  両系統で照会する（当時は照会不能を表示して素通し。v2.54・Phase 56でfail-closedへ是正）。
...
- 是正: `.github/CODEOWNERS` でworkflowディレクトリ全体・integrity検査器・CODEOWNERS自身を
  人間ownerへ割り当てる。一人開発ではPR作成をmachine user/GitHub Appへ分離する。
  ruleset/旧来保護ではcode owner review必須・
...
- 境界: CODEOWNERは人間本人、PR作者はmachine user/GitHub Appとする。AIへ人間ownerの資格情報を
  渡さない。同一GitHubアカウントで人間とAIを動かす運用では両者をサーバー側で区別できないため、
  厳格モードのStep 9を✅にしない（AIにGitHub書込み権限を与えない手動publish運用は縮退案）。

### Phase 54 — v2.52 同梱 ✅（配布元CODEOWNERS分離＋workflow供給網固定）（G5/G7/G9/G13）

- 発見: 配布用placeholder入り `.github/CODEOWNERS` をキット自身でも使っていたため、GitHub上で
  owner解決エラーとなり、保護対象3行が実効しなかった。配布元の実設定を
  `.github/CODEOWNERS`、導入先原本を `.guardrails/CODEOWNERS.template` に分離し、インストーラ
  だけが後者を導入先の前者へ写す。構造検査はplaceholder混入・対象パス差分をhardで拒否する。
- trusted/CI workflowと全catalog列の外部Actionを40桁コミットSHAへ固定。存在しなかった
  `supabase/setup-cli@v1` は公式のv3へ是正した。trusted jobの`pre-commit`本体とremote hookも
  実測版/コミットへ固定し、Actionタグを直しても実行ツールが最新版へ漂う二段目の穴を塞いだ。
- `check_workflow_integrity --verify-scenarios` はtrusted workflowのtrigger・最小permissions・
  fetch/verify command・全外部Action SHAを違反注入で検証する。
- CI YAMLだけでなく、PR headで実行されるpre-commit設定・全scripts・違反コーパス・フック設定・
  Python実行環境もCODEOWNERS対象にした。job名/YAMLを維持したまま検査本体をno-op化する経路を
  同じfresh code owner review境界で止める。配布元ではテンプレート自身も追加で保護する。

### Phase 55 — v2.53 同梱 ✅（CODEOWNERS更新保持＋owner解決実測）（G1/G5/G7/G9）

- 発見: v2.52のCODEOWNERS更新は既存ファイル全体をテンプレートで置換し、独自のアプリ所有規則を
  消した上で、最初のowner 1名へ全パスを平坦化していた。専用管理区画を導入し、独自規則・
  複数owner・パス別ownerを保持する。CODEOWNERSは最後に一致した規則が勝つため、管理区画を
  常に末尾へ配置し、後方の`*`規則による保護上書きも防ぐ。旧版の区画なし形式は一度だけ移行する。
- ローカル表記だけでは`@typo-user`の実在・write権限を判定できない。Step 9外部監査でGitHubの
  `codeowners/errors` APIを照会し、既定ブランチのエラー0件を完了条件にした。GitHubが対応する
  email ownerはローカル検査でも正当に受理する。
- 回帰: 独自規則、複数owner、パス別owner、2回更新の冪等性、後勝ち規則、placeholder残置、
  GitHub owner解決エラーを違反注入シナリオへ追加した。
- 直インストールが`rglob`で作業ツリー全体を配布し、未追跡の計画メモ等を導入先へ漏らす
  経路も発見。Git checkoutでは`git ls-files`の追跡集合だけをマニフェストにし、`.git`の無い
  zip展開物だけ全走査する。未追跡`PLAN.md`の非流出を回帰化した。

### Phase 56 — v2.54 同梱 ✅（Step 9検証不能のfail-closed化）（G7/G9/G10）

- 発見: `verify_required_checks` はStep 9を✅にするアサーションなのに、gh不在・API到達不能・
  ruleset詳細の権限不足を表示だけで成功にしていた。「外部防壁を実測できなければStep 9を
  完了扱いにしない」という契約と逆向きだった。
- 是正: required checks、bypass無しPR、fresh CODEOWNER review、CODEOWNERS owner解決について、
  不在だけでなく検証不能も失敗にする。rulesetsか旧来保護の片方から全条件の正の証拠を
  得られた場合だけ、他方の照会不能を許容する。gh不在・非GitHub remote・offlineもStep 9は赤。
- 回帰シナリオは23本。rulesetだけ/旧来保護だけの正常系を維持しつつ、権限不足・API不通・
  部分確認を成功に戻す変更を拒否する。

### Phase 57 — v2.55 同梱 ✅（Docker/local Actionの参照境界）（G7/G9/G13）

- 発見: 外部Actionの40桁SHA検査が`docker://`を無条件除外し、`image:latest`等の可変イメージを
  許していた。Docker Actionは`@sha256:`＋64桁digestを必須化した。
- `./` local ActionもPR head側の実行コードで、保護ディレクトリ内のActionから保護外コードを
  呼べるため、required CIでは全面禁止する。必要な処理は固定workflowの`run`から明示的に呼ぶ。
- 可変Docker tagとlocal Actionの違反注入を追加し、workflowシナリオは15本になった。

### Phase 58 — v2.56 同梱 ✅（全PRの人間承認境界）（G7/G9/G10）

- 発見: code owner reviewだけを必須にしても、CODEOWNERS対象外のアプリ変更はmachine userが
  人間レビュー0件でmergeできる。PR作者を別アカウントへ分ける運用が、通常変更では人間境界に
  なっていなかった。
- rulesets/旧来保護の両方で`required_approving_review_count >= 1`を要求する。全PRは別アカウントの
  承認1件、ゲート関連パスはさらにCODEOWNER承認が必要。承認後pushは従来どおりstale失効または
  last-push approvalを要求する。
- 承認0件のrulesetを未保護として拒否する。複数のbypassなしrulesetへCODEOWNER・fresh・
  承認件数が分散する正当な構成は、GitHubの実効ルール同様に合成評価する。

### Phase 59 — v2.57 同梱 ✅（required checks側rulesetのbypass監査）（G7/G9/G10）

- 発見: bypass一覧を調べていたのは`pull_request`ルールのrulesetだけで、
  `required_status_checks`を定義する別rulesetはcontext/appだけを見ていた。checks側rulesetに
  machine userをbypass登録すると、4コアjobが登録済みに見えたまま迂回できた。
- required status ruleから`ruleset_id`も取得し、そのrulesetのbypass actorが0件と確認できた
  contextだけを必須jobの証拠に数える。PR/statusで別rulesetの場合も個別に照会・キャッシュする。
- status側だけbypassあり、PR側はbypassなしの違反注入を追加。bootstrapシナリオは26本。

### Phase 60 — v2.58 同梱 ✅（classic checksのadmin適用監査）（G7/G9/G10）

- 発見: 旧来ブランチ保護の`enforce_admins`はPRレビュー設定が存在する場合だけ確認し、
  required status checksだけをclassic側から採る構成ではadmin非適用でも必須contextに数えていた。
- classic required checksを証拠に使う場合もbranch protection全体の`enforce_admins.enabled=true`を
  必須化。PRレビューとstatus checksで同じ照会結果を共有し、権限不足は検証不能として失敗する。
- classic checksあり・admin非適用、ruleset PRは安全という分離構成を追加。シナリオは27本。

### Phase 61 — v2.59 同梱 ✅（インストーラのGit index隔離）（G1/G7/G9）

- 発見: 一時indexで全ゲートを走らせると`GIT_INDEX_FILE`が子プロセスへ継承され、直インストールの
  `git ls-files`がkit自身でなく呼出元indexを読む。未追跡非流出テストも別repositoryのindexを
...
### Phase 63 — v2.61 同梱 ✅（慢性化したsoft違反の検出 `chronic-soft-violation`）（G3/G9）

- 経緯: §1-3 の規約をファイル500行/フォルダ7ファイルという機械分割の軸から関心事の軸へ
  再定義した（AGENTS.md §1/§2・Phase 62 直後の改訂）。500行/7ファイルは「1回で読めるか」
  「`ls`一覧性」の代理指標であり目的ではないため、soft警告は理由があれば1回は見送って
  よいという運用に変わった——その代償として、無理由のまま何コミットも同じ箇所が
  肥大し続ける経路に歯止めが無くなる懸念があった。
- `.guardrails/violations.jsonl`（§3.6・gitignore済みのローカル台帳）は `check-structure` の
  実行ごとに全 finding を記録しているため、追加インフラなしで「同じ箇所が何回の実行に
  またがって soft を出し続けているか」を数えられる。`file-too-long` / `dir-too-crowded`
  それぞれについて (規則ID, 発生箇所) をキーに実行のタイムスタンプ集合を数え、
  `CHRONIC_SOFT_THRESHOLD`（既定5）以上にまたがったら `SOFT:chronic-soft-violation` を
  追加で出す。あくまで soft のまま——「関心事として正当」という人間判断を機械が覆さない。
- 発見（実装直後の dogfooding で実測）: 初版は台帳の記録だけを数えており、キット自身の
  リポジトリで実行した初回実測で「削除済みの検証用フォルダ（過去の dir-too-crowded 手動
  プローブの残骸）」を23回の履歴があるという理由で誤検知した。台帳は追記専用で解消済みの
  記録も残るため、履歴だけを見ると**直した箇所も直り続けている限り鳴り続ける**——
  これは「慢性の放置」ではなく検査側の欠陥。**今回の実行で file-too-long /
  dir-too-crowded が現に鳴っている箇所だけ**に対象を絞り、履歴が長くても現在解消済みなら
  即座に不発にする形へ是正した（`check_soft_limits` が確定させた今回の findings を
  `check_chronic_soft_violations` が受け取り、その集合との積を取るだけ——追加の走査は
  増やさない）。
- 台帳はローカル・gitignore済みのため、新規clone・CI では毎回空から始まり不発になる。
  これは既存の `binding-unstamped` 等と同じ「見える猶予」の設計であり、意図的な非対称
  （長期間同じ開発機で作業を続けるほど効く——短命な CI 実行のための検査ではない）。

### Phase 64 — v2.63 同梱 ✅（doc影響の未申告検出 `doc-impact-undeclared`）（G9）

- 発端: 人間向け文書を HTML 化した直後（Phase §4 文書書式規約・v2.62）でも、
  「利用者影響のあるソース変更をしたのに、対応する文書を更新し忘れる」という事象は
  機械検査の対象外のままだった——doc の陳腐化は commit の diff に出ないため、
  既存の hard/soft 検査（fix⇔テスト・feat⇔plan 等）のどれも拾わない空白だった。
...
- 免除は他の存在検査（`NO-LOG:` / `RED-FIRST-EXEMPT:` / `NONDETERMINISM-EXEMPT:`）と同じ
  ...
  新設しない（重複登録しない — G5）。
- **soft 導入**（hard にしない理由）: 対応表の精度（source/docs パターンの粒度）は
  採用先ごとの実測でしか検証できない。粒度を誤ると偽陽性が鳴りっぱなしになり、無視される
  soft より悪い「無視される hard」になり得る（G9 が守ろうとしている沈黙禁止それ自体を
  hard の形骸化で裏切る）——feat-without-plan と同じ非対称閾値（.guardrails/GOALS.md
  運用ルール）に従い、実測で偽陽性率を確認してから hard 昇格を判断する。
- 対象外として記録した範囲: 「コードを変更していないコミットで doc が現実と食い違って
  いく」型（撤回済みの結論が文書に残り続ける・状態欄が実装とずれる 等）はこの検査の対象
...
コピーせず `scripts/install_kit.py` で配置する（README_SETUP.html §1 が正本。既存ファイルは決して
...
- 完了条件: ① 正常 PR で全ジョブ緑 ② **GitHub の Web エディタから**違反を1件コミット
  ...
  **4コアジョブ（`checks`・`red-first`・`commit-msg-history`・`workflow-integrity`）と採用した
  ...
  check-bootstrap が `gh api` で実測検証する**——不在・検証不能のどちらでも本 Step は
  ✅ にできない — §3.5・v2.35〜v2.54）。登録は **rulesets 側を推奨**
  （CI の GITHUB_TOKEN で照会できるのは rulesets のみ＝CI 再監査が確定判定になる — v2.36）。
  全PRで承認1件以上を要求し、bypass主体を置かず、旧来保護を使う場合はadminにも強制する。
  workflow更新だけは人間レビュー後に
  `workflow-integrity` のrequired指定を一時解除し、同じPRをmerge後ただちに再有効化する。
  インストールされた `.github/CODEOWNERS` のplaceholderを人間ownerへ置換する（キット配布元の
  実設定と配布原本 `.guardrails/CODEOWNERS.template` は分離済み）。一人開発ではPR作成を権限を絞った
  machine user/GitHub Appへ分離し、workflow群・
  workflow・ゲート設定/スクリプト/コーパス・CODEOWNERS自身へのcode owner reviewも
  サーバー側で必須化する。
  ...
  実測する（CIで項目が省略された場合は0人と断定しない）。同時に
  `gh api repos/{owner}/{repo}/codeowners/errors` が0件であることも検査する。CODEOWNERSは
  PRのbase branch版が使われるため、新規導入・owner変更は既定ブランチ反映後に再実測する。
