...
- **`scripts/check_bootstrap.py`**（pre-commit の `check-bootstrap`——`files: ^\.guardrails/BOOTSTRAP\.md$`
  ...
  - `bootstrap-false-done`: **✅ の Step ごとに、検証可能なアサーションをその場で再実行**
    ...
    **不在の断定は両系統の確定回答が揃った時のみ**（v2.36 是正——片系統が照会不能のまま
    ...
    強制最低線は `red-first` のみ（親で赤の証明はこのジョブしか担わない）——`checks` /
    `commit-msg-history` の未登録は表示で推奨に留める。回帰は
    `check_bootstrap.py --verify-scenarios`（全モック8本・pre-commit の
...
- **是正（v2.36——外部レビュー起点の再検証で発見した偽陽性）**: 初版は「どちらか一方でも
  ...
  **必ず偽赤**になる構成だった。修正: 発見すれば片系統でも合格（存在証明）・不在の断定は
  ...
  「門番自身の回帰」の型）で固定した。シナリオ4が本偽陽性の再発防止。あわせて
  `checks` / `commit-msg-history` の required 未登録を表示のみで可視化（強制最低線は
  red-first——親で赤の証明はこのジョブしか担わず、他2つはローカル門と重なる再実行のため
  要求水準はプロジェクト判断とする——判断ごと記録）。
...
- 完了条件: ① 正常 PR で全ジョブ緑 ② **GitHub の Web エディタから**違反を1件コミット
  ...
  **登録の有無は check-bootstrap が `gh api` で実測検証する**——検証不能環境は表示して
  素通し・検証できて不在なら本 Step は ✅ にできない — §3.5・v2.35）。登録は
  **rulesets 側を推奨**（CI の GITHUB_TOKEN で照会できるのは rulesets のみ＝CI 再監査が
  確定判定になる — v2.36）。`checks` / `commit-msg-history` も required へ入れるのを
  推奨（強制最低線は red-first——未登録は表示のみ §3.5）。
