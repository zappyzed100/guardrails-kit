...
- **BINDING 充填のマーカー包み＋機械移植**（G13——v2.38 登録）: 更新時の充填復元
  （PROMPT_claude_code_update Step U2）は現状エージェントの意味マージ。充填 paste-block を
  `# >>> COLUMN 列ID@版 >>>` 〜 `# <<< COLUMN <<<` で包む規約に変えれば、インストーラが
  UPGRADED 時に**マーカー区画だけ**を新版へ機械移植でき、復元工数と事故面が消える
  （gitignore のキット区画マーカーと同じ型）。要る変更: カタログ全列の paste-block・
  §11 Step 0 の充填手順・インストーラの移植実装。トリガー = 更新プロンプト運用で
  **復元漏れ・復元事故が実測された時**（それまでは git 履歴＋check の赤が同役を担う）。
