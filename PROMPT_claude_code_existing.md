...
完了条件の正本はリポジトリ直下の `GUARDRAILS.md`。まず全文を読み、**§11 の Step 0→10 を、
...
1. **既存コードには違反が眠っている**。全 hard 規則を即時有効化すると、その瞬間から
   ...
   違反が残る規則は BINDING で一時停止し、**必ず GUARDRAILS.md §10 に清掃 Phase として
...
- **Step 0（入力確定）**: 棚卸し結果＋上の「入力」から表A/B/Cの全セルを埋める。加えて
  **「規則ID × 違反件数 × 方針（即時有効 / Phase 送り）」の判定表**を作り、GUARDRAILS.md に
...
- **Step 2（スクリプト調整と初回生成）**: BINDING を Step 0 の判定表どおりに確定
  ...
  **有効化した全規則**への違反注入・2秒以内）＋「一時停止した規則が §10 に全件 Phase 登録
  されている」こと。
...
- **Step 7（ログ単一出口）＝清掃 Phase の代表例**: ①単一出口を実装 ②**既存の print 系
  ...
  ④違反注入で落ちるのを実測——を**1つの PR** で行う（検査だけ足して移行しないのは禁止。
...
（チェックリスト全✅＋違反件数の増減表＋残 Phase 一覧）を報告し、**`CUSTOMIZE.md` の
