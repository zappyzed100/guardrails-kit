...
#   の各行から {"tool_input":{"command": …}} を組み立て、guard_git_bypass.sh へ
...
#   / exit 2 = 内部エラー（コーパス消失・書式不正・bash/jq 不在・guard の想定外 exit 等）
#
# probe:
...
#
# 前提ツール（§7.2 の流儀: 静かなスキップ禁止・不在は明示エラー）:
#   - bash … guard 本体の実行に必要（Windows は Git for Windows 付属の bash）。
#   - jq   … コーパスの期待値は guard の精密経路（引用符除去 — §2）前提。jq 不在時の
...
GUARD_REL = ".claude/hooks/guard_git_bypass.sh"
...
def run_guard(bash: str, guard: Path, command: str, cwd: Path | None = None) -> tuple[str, str]:
...
def replay(bash: str, guard: Path, rows: list[tuple[int, str, str | None, str]]) -> int:
...
def probe(bash: str, guard: Path, command: str) -> int:
    actual, reason = run_guard(bash, guard, command)
    if actual == "ALLOW":
        print("ALLOW")
        return 0
    if actual == "DENY":
...
def main(argv: list[str]) -> int:
    ...
    bash = resolve_tool(
        "bash", "guard はシェルスクリプト（Windows は Git for Windows 付属の bash を PATH へ）")
    resolve_tool(
        "jq", "コーパスの期待値は guard の精密経路（引用符除去）前提のため必須"
        "（jq 不在の保守的経路は ALLOW 行を過剰ブロックし偽の不一致になる — §2）")

    if args.probe is not None:
        if not args.probe.strip():
            raise rs.ScanError("probe のコマンドが空")
        return probe(bash, guard, args.probe)
    return replay(bash, guard, load_corpus(root / CORPUS_REL))
