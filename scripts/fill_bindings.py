...
例: uv run scripts/fill_bindings.py python-uv@9
    uv run scripts/fill_bindings.py dart-flutter@7 rust@8   # 併用（加算形が前提）
...
def main() -> int:
    ...
    for column in args.columns:
        ...
        for target_rel, block in blocks:
            ...
            if status == "FILLED":
                filled_any = True
            elif status in ("NO-REGION", "MISSING-TARGET"):
                failed = True
    ...
    if failed:
        print("\nfill_bindings: 未適用あり（上の行を解消して再実行——冪等）")
        return 1
    ...
    print(f"\nfill_bindings: 完了{tail}。次: `uv run scripts/check_structure.py` で"
          ...
          + ("" if not filled_any else "。YAML 系（.pre-commit-config.yaml / CI）の列固有"
             "フックはカタログの該当 paste-block を手で統合する（機械充填の対象外——判断の"
             "記録は SURVEY_HARNESS_TOOLS.md §1）"))
    return 0
