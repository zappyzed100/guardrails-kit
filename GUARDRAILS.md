...
| セッション開始→編集直前（SessionStart + PreToolUse Edit/Write/MultiEdit） | 所有権ガード（§2c・v2.6）——人間の未コミット変更の上書き防止 | × | —（fail-open＋表示） | `.claude/settings.json`・`.claude/hooks/session_baseline.sh`・`guard_human_wip.py` |
...
- 経緯: 「他にも同種の遅い実装が無いか」「そもそもbash採用に根拠はあるか」という
  ユーザーの問いを受け、全フックを棚卸しし、旧 `guard_git_bypass.sh`（v2.22是正後でも
...
- 境界（今回やらなかったこと・理由）: `stop_incomplete_guard.sh`・`session_baseline.sh`・
  `post_edit_format.sh`・`post_edit_lint.sh` は未調査/未移行。前2者はEdit系ホットパスほど
  高頻度ではなく（Stop試行毎・セッション開始1回）、後2者は元々サブプロセス数が少ない
  （§12.1 dev.py 動詞への委譲が主体）——優先度が本4スクリプトより低いと判断し、
  今回のセッション予算内では手を付けなかった。次に着手する時は同じ型（実測→corpus/
  手動比較で検証→ドキュメント更新）を踏襲する。
...
- 作る: `.claude/settings.json`（`permissions.deny`: `--no-verify` / `--force` push /
  ...
  同梱済み（v2.6）: `.claude/hooks/session_baseline.sh`＋`guard_human_wip.py`
