...
- 境界（このコミット時点でやらなかったこと）: `stop_incomplete_guard.sh`・
  ...
  元々サブプロセス数が少ないという理由）。**ただしユーザーから「統一した方がよい」
  という判断が入り、同一セッション内で Phase 34 として実施した**——見送りの理由自体は
  誤りではなかったが、統一の価値（言語1つに揃える保守性）を過小評価していた。
...
- 経緯: Phase 33 で見送った4フックについて、ユーザーから「統一した方がよい。実装にも
  時間はかからないはず」という判断が入り、同一手順（実測→移植→検証→配線）で実施。
  加えて、「フック本体をPython化しても、呼び出すruff/prettier等が遅ければ意味が薄い」
  という指摘を受け、`post_edit_format.py`/`post_edit_lint.py` が呼ぶ外部ツールの
  呼び出し方も見直した。
