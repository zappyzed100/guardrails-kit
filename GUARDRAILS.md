...
|---|---|---|---|---|
| 編集直後（Edit/Write/MultiEdit） | 採用列の整形→単一ファイル lint の**直列2段**（§1・v2.5） | 整形○／lint× | — | `.claude/settings.json`・`.claude/hooks/post_edit_format.sh`・`post_edit_lint.sh` |
...
| ターン終了（Stop） | 未完了終了の差し戻しゲート（§2b。条件A=未コミット作業・条件B=構造検査が赤 — v2.9）——実行規律7の機械化 | × | —（fail-open＋回数上限） | `.claude/settings.json`・`.claude/hooks/stop_incomplete_guard.sh` |
| セッション開始→編集直前（SessionStart + PreToolUse Edit/Write/MultiEdit） | 所有権ガード（§2c・v2.6）——人間の未コミット変更の上書き防止 | × | —（fail-open＋表示） | `.claude/settings.json`・`.claude/hooks/session_baseline.sh`・`guard_human_wip.sh` |
...
- **`.claude/hooks/post_edit_format.sh`（第1段・自動修正系）** — Edit/Write/MultiEditの直後に走る。対象判定は
  編集されたファイルパスの case 分岐で行い、**採用列の整形コマンド**をその場で当てる
  （case の中身は `bindings/catalog.md` の paste-block を Step 0 で充填。整形は冪等であること）。
- **`.claude/hooks/post_edit_lint.sh`（第2段・判定系 — Phase 12）** — 整形の直後、同じ編集
...
- **`.claude/hooks/guard_git_bypass.sh`**（PreToolUse: Bash） — 実行されようとする Bash
...
- **`.claude/hooks/stop_incomplete_guard.sh`**（`settings.json` の `hooks.Stop`）——
...
- **`.claude/hooks/session_baseline.sh`**（`hooks.SessionStart`）——セッション開始時点の
...
- **`.claude/hooks/guard_human_wip.sh`**（PreToolUse: `Edit|Write|MultiEdit`）——ブロック
...
- guard_human_wip.sh は §2 のコーパス再生の**対象外**（別フック・baseline という状態を
...
- ✅ `context-doc-too-large`（**soft**） — 常時読込の規約文書（ルート/フォルダ CLAUDE.md
  ...
  の行数超過を警告（v2.17・Phase 27・調査③）。常時読込の行数はそのまま常駐コンテキスト
...
ファイルで約60ms）。guard コーパス再生は全行2秒以内（§2。並列度は `os.cpu_count()` から
自動導出——v2.6・G11。標準ライブラリで Windows 含め動くため、ユーザー入力も調査
スクリプトも不要。行数増で超過したら並列度→guard 内のプロセス起動回数の順に削る——
計測は実機・複数コア前提。1コアのサンドボックス実測は予算対象外）。
予算超過の第一容疑者は常に「プロセス起動回数」と「O(N²)」（§7.3）。
...
| 27 | context-doc-too-large（soft——調査③の採用。Skills 化保留のセンサー） | §3.3 | ✅（v2.17 同梱） |
...
- 契約と正本は §3.3 `deprecated-api`（BINDING は `repo_scan.py` の `DEPRECATED_PATTERNS`・
  出典規律はカタログ注記）・§2c（所有権ガード——`session_baseline.sh`＋`guard_human_wip.sh`・
...
- 動機: 「LLM がプロンプト1つでキットを展開できるか」への懐疑（ユーザー提起）。
...
### Phase 27 — v2.17 同梱 ✅（context-doc-too-large・soft——調査③の採用）（G3/G9）
...
- **所有権ガードのコーパス再生**（G10）: `guard_human_wip.sh` は §2 コーパスの対象外
  （別フック・baseline という状態を持つ）。現状の回帰担保は Step 4 の違反注入 DoD。
  トリガー = guard_human_wip.sh の**改修**が発生した時（`check_guard_corpus.py` に
  `--hook` 引数を拡張し、baseline フィクスチャ込みで再生する——§2c）。
...
- 作る: `.claude/settings.json`（`permissions.deny`: `--no-verify` / `--force` push /
  ...
  `.claude/hooks/guard_git_bypass.sh`（exit 2・fail-closed。--no-verify/-n・SKIP=・
  --force/-f push・core.hooksPath を検出——§2）・
  `.claude/hooks/post_edit_format.sh`（A の整形コマンドで対象拡張子を判定——§1）・
  `.claude/hooks/post_edit_lint.sh`（A の「編集直後 lint」を充填——§1 第2段。v2.5。
  ...
  `.claude/hooks/stop_incomplete_guard.sh`（ターン終了ゲート——§2b）。
  同梱済み（v2.6）: `.claude/hooks/session_baseline.sh`＋`guard_human_wip.sh`
...
- **刻印**: 採用列を決めたら、対象ファイル（`repo_scan.py`・`dev.py`・
  `.pre-commit-config.yaml`・`guardrails-ci.yml`・`post_edit_format.sh`）のヘッダーに
