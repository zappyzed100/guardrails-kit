...
1. 順序固定・スキップ禁止。**全 Step で** 1 Step = 1 コミット = 1 ブランチ = 1 PR。
   既定ブランチへ直接 push しない（PR の required checks が、フックの無い変更を合流前に
   止める唯一の防壁であり、push 後 CI では代替できない）。
...
- **Step 3**: `uv tool install pre-commit==4.6.0` → `pre-commit install` → 違反注入コミット
...
- **Step 8**: 非決定パターン検査は有効化済み（発火の実測はコーパス対象なら
  `uv run scripts/dev.py dod` が一括で証明——test-sleep / test-nondeterminism /
  test-network 等。手でやり直さない — 実行規律6）。確率的コンポーネント
...
- **Step 9**: `guardrails-ci.yml` は配置済み。ツールチェーン版の固定を確定し、リモートへ push。
  ...
  しない）→ 検証ブランチ削除。サーバー側でPR必須・bypassなしを設定し、4コアjob
  （`checks` / `red-first` / `commit-msg-history` / `workflow-integrity`）と採用した全言語別jobを
  requiredにする。`.github/CODEOWNERS` のplaceholderを入力欄の人間ownerへ置換し、PR作成は
  owner権限を渡していないmachine user/GitHub Appに行わせる。workflow群・
  ...
  workflow更新時だけはその人間レビュー後に `workflow-integrity` を一時解除し、
  同じPRをmerge後ただちに戻す（直接pushはしない）。
- **Step 10**: 総合セルフ監査（.guardrails/GUARDRAILS.md §11 Step 10 のとおり: チェックリスト全行の
  同一コミット✅・TODO 0件・固有名詞 0件・**全規則IDの違反注入実績**の突き合わせ
  （コーパス対象は `dev.py dod` の PASS 一覧がそのまま実績・手動は対象外5分類の分のみ
  — Phase 47）・
  残項目の §10 Phase 登録）。
